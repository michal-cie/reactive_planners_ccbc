...
des_contacts = [pd_contact_des_contact, pd_two_contact_des_contact, pd_des_vel_des_contact]
act_contacts = [pd_contact_act_contact, pd_two_contact_act_contact, pd_des_vel_act_contact]
...
contact_errors = [pd_contact_contact_error , pd_two_contact_contact_error, pd_des_vel_contact_error]
contact_stds = [[],[],[]]
box_pd_contact_vel_error = []
box_pd_two_contact_vel_error = []
box_pd_des_vel_contact_vel_error = []
box_errors = [box_pd_contact_vel_error,box_pd_two_contact_vel_error,box_pd_des_vel_contact_vel_error]
...
for i in range(5):
    for j in range(3):
        ...
        box_errors[j].append(error)
        contact_stds[j].append(np.std(abs(error)))
        contact_errors[j].append(abs(error).mean())
...
# rects3 = ax.bar(index + 2 * bar_width, pd_two_contact_fails, bar_width, alpha=opacity, label='Two Contact')
# rects4 = ax.bar(index + 3 * bar_width, pd_two_contact_gait_fails, bar_width, alpha=opacity, label='Two Contact Gait')
# rects5 = ax.bar(index + 4 * bar_width, pd_des_vel_fails, bar_width, alpha=opacity, label='Desired Velocity')
rects1 = ax.bar(index, contact_errors[0], bar_width, alpha=opacity, yerr=contact_stds[0],label='Contact', capsize=5)
# rects2 = ax.bar(index + bar_width, contact_errors[1], bar_width, alpha=opacity, label='Contact Gait')
rects2 = ax.bar(index + 1 * bar_width, contact_errors[1], bar_width, alpha=opacity, yerr=contact_stds[1], label='Two Contact', capsize=5)
# rects4 = ax.bar(index + 3 * bar_width, contact_errors[3], bar_width, alpha=opacity, label='Two Contact Gait')
rects3 = ax.bar(index + 2 * bar_width, contact_errors[2], bar_width, alpha=opacity, yerr=contact_stds[2], label='Desired Velocity', capsize=5)

# Add labels, title, and legend
ax.set_xlabel('Rollouts in Training Data')
ax.set_ylabel('Distance (m)')
ax.set_title('Contact Position Error by Policy Type')
ax.set_xticks(index + 1 * bar_width)
ax.set_xticklabels(['25','50','100','200','400'])
ax.legend()
# ax.set_xlabel('Rollouts in Training Data')
# ax.set_ylabel('Number of Fails')
# ax.set_title('Fails by Policy Type and Rollouts in Training Data')
...
plt.savefig(f"{eval_data_dir}/contact_errors.png")
...
box_pd_contact_err = box_errors[0]
box_pd_two_contact_err = box_errors[1]
box_pd_des_vel_err = box_errors[2]

positions1 = np.arange(len(box_pd_contact_err)) * 2.0 - 0.6
positions2 = np.arange(len(box_pd_two_contact_err)) * 2.0
positions3 = np.arange(len(box_pd_des_vel_err)) * 2.0 + 0.6
bp1 = ax.boxplot(box_pd_contact_err,positions=positions1, patch_artist=True,
...
bp2 = ax.boxplot(box_pd_two_contact_err,positions=positions2, patch_artist=True,
...
bp3 = ax.boxplot(box_pd_des_vel_err,positions=positions3, patch_artist=True,
...
ax.set_ylabel('Distance (m)')
ax.set_title('Contact Position Error by Policy Type')
ax.set_xticks(np.arange(len(box_pd_two_contact_err)) * 2.0)
ax.set_xticklabels(ticks)
ax.legend([bp1["boxes"][0], bp2["boxes"][0], bp3["boxes"][0]], ['Contact', 'Two Contact', 'Desired Velocity'], loc='upper left')

plt.savefig(f"{eval_data_dir}/contact_errors_box.png")
plt.show()
plt.close()